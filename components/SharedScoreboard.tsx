...
export default function SharedScoreboard({ 
  ...
  // FETCH DATA: Uses the props (csvUrlLower/Upper) instead of hardcoded links!
  const fetchData = useCallback(async (manual = false) => {
    ...
    try {
      ...
      const textLower = await resLower.text();
      const textUpper = await resUpper.text();

      const dataLower = parseCSVData(textLower);
      const dataUpper = parseCSVData(textUpper);
  ...
  return (
    ...
    <div className={`h-screen w-screen overflow-hidden ${bgColor} p-4 lg:p-6 flex flex-col items-center transition-colors duration-500`}>
      
      <div className="flex justify-between items-center w-full max-w-[1400px] mb-4 shrink-0 px-2">
        <div>
        ...
        <div className="flex items-center gap-3 bg-white/10 backdrop-blur-sm px-4 py-2 rounded-xl border border-white/20">
          ...
          <button 
            ...
            className={`p-2 bg-white text-gray-900 rounded-lg shadow-md flex items-center justify-center ${isRefreshing ? 'opacity-70 cursor-not-allowed' : 'hover:scale-105 active:scale-95'}`}
          >
            <RefreshCw className={`w-4 h-4 lg:w-5 lg:h-5 ${isRefreshing ? 'animate-spin text-gray-400' : ''}`} />
          </button>
        </div>
      </div>

      <div className="w-full max-w-[1400px] flex-1 min-h-0 pb-2 px-2">
        <ul className="grid grid-cols-1 grid-flow-row lg:grid-cols-2 lg:grid-rows-6 lg:grid-flow-col gap-x-8 gap-y-3 h-full content-start">
          <AnimatePresence>
            {sortedHouses.map((house, index) => {
              ...
              return (
                <motion.li
                  ...
                  className={`relative flex items-center justify-between px-4 py-2 lg:px-5 lg:py-3 rounded-xl shadow-md border-l-8 bg-white h-[76px] lg:h-[88px] ${
                ...
                >
                  <div className="flex items-center gap-4 overflow-hidden">
                    <div className={`flex-shrink-0 w-12 h-12 lg:w-14 lg:h-14 flex items-center justify-center rounded-full text-xl font-black shadow-inner ${
                    ...
                    <div className="flex flex-col min-w-0">
                      <h2 className={`text-xl lg:text-2xl font-black tracking-wide mb-1 truncate ${isTop1 ? 'text-gray-900' : 'text-gray-800'}`}>
                        {house.name}
                      </h2>
                      <div className="flex gap-2 text-[10px] lg:text-xs font-bold text-gray-500 truncate">
                        <span className="bg-gray-50 text-gray-700 px-2 py-0.5 rounded border border-gray-200">
                  ...
                  <div className="flex flex-col items-end justify-center w-24 lg:w-32 shrink-0 border-l border-gray-100 pl-4">
                    <p className="text-[9px] lg:text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">Total</p>
